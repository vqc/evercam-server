defmodule EvercamMedia.CameraShareRequestControllerTest do
  ...
  setup do
    ...
    camera2 = Repo.insert!(%Camera{owner_id: user.id, name: "Camera xyz", exid: "cameraxyz", is_public: false, config: ""})
    share_request = Repo.insert!(%CameraShareRequest{camera_id: camera.id, user_id: user.id, key: UUID.uuid4(:hex),
      email: "abc@email.com", status: -1, rights: "list,snapshot"})
